...
import {
    ...
    TouchableHighlight, Linking, ScrollView
} from 'react-native';
...
import ViewDetailsReducer from "./ViewDetailsReducer";
...
class ViewDetailsScreen extends Component {
    constructor(props) {
        super(props);
    ...
    componentWillMount() {
        ...
        var image_arr = [];
        for (var i in item.all_images) {
            if (i < 4) {
                image_arr.push('http://www.nytimes.com/' + item.all_images[i].url);
            }
        }
        if (image_arr.length === 0) {
            image_arr.push('https://st2.depositphotos.com/4740575/7202/v/600/depositphotos_72029957-stock-video-world-news-background.jpg');
        }
    ...
    render() {
        ...
        const date = moment(item.date || "").format("MMM Do YYYY, h:mm:ss a");

        let shareOptions = {
            title: "Eurisko",
            message: item.web_url,
            ...
            <ScrollView>
                <View style={styles.container}>
                    <View style={styles.content1}>
                        <Text style={styles.contentText}>{item.headline}</Text>
                    </View>
                                ...
                                {images.map((image, index) => {
                                    return (
                                        <TouchableHighlight
                                            ...
                                            underlayColor="#ccc"
                                            onPress={() => move(index)}
                                            style={position === index ? styles.buttonPressed : styles.button}
                                            elevation={5}
                                        >
                    ...
                    <View style={styles.content2}>
                        <Text style={styles.contentTextByline}>{item.byline}</Text>
                        {item.subsection_name ?
                            <Text style={styles.contentTextSubsection}>{item.subsection_name}</Text> :
                            <View style={{margin: 0, padding: 0}}></View>}
                        <Text style={styles.contentText2}>{item.lead_paragraph}</Text>
                    ...
                    <View style={styles.content2}>
                        ...
                        >
                            <View>
                                <Text style={styles.contentText2}>Link:
                                    <Text style={{color: 'blue'}}
                                          onPress={() => Linking.openURL(item.web_url)}> Visit Link
                                    </Text>
                            ...
                            <View>
                                <TouchableOpacity style={{color: 'blue'}}
                                    ...
                                    <Image
                                        ...
                                        source={require('gallery/share-symbol.png')}
                                    />
                                </TouchableOpacity>
                            </View>
                        </View>
                        ...
                        >
                            {item.source === "The New York Times" ?
                                <Text style={styles.noteStyle}>{item.source}</Text>
                                :
                                <Text style={styles.noteStyle}>Source: {item.source}</Text>
                            }
